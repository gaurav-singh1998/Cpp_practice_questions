...
void BuildTree(vector<vector<ll>> &tree,vector<ll> arr,int s,int e,int index)
{
    ...
    // vector<ll> first=tree[2*index];
    // vector<ll> second=tree[2*index+1];
    // tree[index]=vector<ll> (tree[index*2].size()+tree[index*2+1].size());
    // merge(tree[index*2].begin(),tree[index*2].end(),tree[index*2+1].begin(),tree[index*2+1].end(),tree[index].begin());
    merge(tree[2*index].begin(),tree[2*index].end(),
          tree[2*index+1].begin(),tree[2*index+1].end(),back_inserter(tree[index]));
    return;
